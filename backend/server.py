...
async def submit_greeting(
    ...
    if captcha_check["requires_captcha"]:
        ...
        try:
            ...
            if provided_answer_hash != expected_answer_hash:
                ...
                await track_submission_attempt(slug, client_ip, device_id, "wishes", False, True)
                await db.captcha_challenges.delete_one({"id": captcha_id})
                raise HTTPException(
                    status_code=400,
    ...
    await track_submission_attempt(slug, client_ip, device_id, "wishes", True, captcha_check["requires_captcha"])
...
async def submit_rsvp(
    ...
    if captcha_check["requires_captcha"]:
        ...
        try:
            ...
            if provided_answer_hash != expected_answer_hash:
                # Wrong answer - track failed attempt
                await track_submission_attempt(slug, client_ip, device_id, "rsvp", False, True)
                await db.captcha_challenges.delete_one({"id": captcha_id})
                raise HTTPException(
                    status_code=400,
                    detail="Incorrect CAPTCHA answer. Please try again."
    ...
    # PHASE 32: Track successful submission (resets CAPTCHA requirement)
    await track_submission_attempt(slug, client_ip, device_id, "rsvp", True, captcha_check["requires_captcha"])
...
async def generate_qr_code(slug: str):
    ...
    invitation_url = f"https://tie-the-knot-46.preview.emergentagent.com/invite/{slug}"
